...
def generate_external_data_boundary(
    ...
    *,
    ...
    if mapping_paths is None or not mapping_paths.json_path.exists():
        raise ExternalDataBoundaryInputError(f"找不到 industry chain mapping：{podcast_id}/{episode_ref}")
    ...
    if mapping_mode != SUPPORTED_MAPPING_MODE:
        raise ExternalDataBoundaryInputError(f"industry chain mapping mode 不支援：{mapping_mode}")

    mapping_status = _required_text(mapping_payload, "mapping_status")
    if mapping_status == "partial-draft" and not allow_partial:
        raise ExternalDataBoundaryInputError("industry chain mapping status is partial-draft；請使用 --allow-partial。")
    if mapping_status not in {"final", "partial-draft"}:
        raise ExternalDataBoundaryInputError(f"industry chain mapping status 不支援：{mapping_status}")
...
def _load_mapping_payload(json_path: Path) -> dict[str, Any]:
    try:
        ...
        raise ExternalDataBoundaryInputError(f"industry chain mapping JSON 格式錯誤：{json_path}") from exc
    except OSError as exc:
        raise ExternalDataBoundaryInputError(f"無法讀取 industry chain mapping：{exc}") from exc
...
def _validate_mapping_identity(payload: dict[str, Any], podcast_id: str, episode_ref: str) -> None:
    ...
    if payload_podcast_id != podcast_id or payload_episode_ref != episode_ref:
        raise ExternalDataBoundaryInputError("industry chain mapping 的 podcast_id 或 episode_ref 不符合請求。")
    ...
    try:
        ...
        for part_path in (json_part_path, markdown_part_path):
            try:
                ...
                pass
        raise ExternalDataBoundaryFailedError(f"寫入 external data boundary 失敗：{exc}") from exc
