...
def semantic_summarize_episode(
    ...
    *,
    ...
    if max_segments_per_chunk < 1:
        raise ValueError("max_segments_per_chunk must be greater than 0.")
    ...
    try:
        ...
        raise SemanticSummaryFailedError(f"Semantic summary generation failed: {exc}") from exc
...
def _write_summary(summary_path: Path, markdown: str) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        raise SemanticSummaryFailedError(f"Failed to write the semantic summary: {exc}") from exc
