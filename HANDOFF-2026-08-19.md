...
`main` is at `80a4feb` (038 then 039), two commits ahead of `origin/main`.
Do not treat push as pending unless asked.

A–E are done in the working tree: C (`gb10` unavailable / `pro4500`),
D (040 / Tool 23), E (finance synthesis gate). Spec 041 (Tool 24
`ingest_youtube_video`) is implemented in the same uncommitted tree.
Task F / Hermes 026–034 is untouched. Next candidate is a separate
`05`/`06` derivation spec after 038, not another ingest MCP tool and not
Hermes. Do not treat the prototype's eight files as one artifact family.
Do not casually start Hermes.
