...
- **F-03（resolved, Batch 3A）**：exact ack 已在 core-level 強制 — `llm_provider.create_provider` 新增 keyword-only `api_cost_ack`，建構任何 provider 前驗證；`semantic_summarize_episode` 新增 keyword-only `api_cost_ack`，於進入點（所有 early return 之前）驗證。Migration note：直接呼叫這兩個 function 的 library caller 必須傳入 exact ack，否則 raise `LLMProviderConfigError`（fail closed）。殘餘風險：直接建構 `OpenAICompatibleProvider` 可繞過 factory gate，src/ 內無此用法。
