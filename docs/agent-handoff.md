...
The local stdio registry has exact 17 reviewed tools. Tool 17, `query_verified_research_report_catalog`, is append-only; Tools 1–16 contracts and order are preserved. It is a read-query, not a side-effect, and requires neither `confirm` nor acknowledgement.
...
| MCP JSON envelope | MCP responses 維持既有 envelope：`{"ok": true, "data": ...}`、`{"ok": false, ...}`、dry-run 含 `"dry_run": true`；恰好 17 個 reviewed tools | `tests/test_mcp_tool_registry_contract.py`、`tests/test_mcp_server.py` |
