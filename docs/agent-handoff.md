...
The local stdio registry has exact 18 reviewed tools. Tool 18, `revalidate_verified_research_report_sources`, is append-only; Tools 1–17 contracts and order are preserved. It is a read-query, not a side-effect, requires neither `confirm` nor acknowledgement, accepts only the exact locator, and performs no writes. Tool 17 retains its catalog contract.
