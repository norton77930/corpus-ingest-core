...
> The Hermes sidecar audit chain (specs 026-034) terminated at BLOCKED and was removed from `main` on 2026-08-29. Its full record is preserved under the git tag `archive/hermes-audit-chain`.
...
- `026`–`034` were the Hermes sidecar audit chain, and the chain **terminated at BLOCKED**: 026 never obtained C7 (safe Hermes runtime-routing evidence), 029–032 stayed offline-only at `BLOCKED_RUNTIME_SEAM` then `BLOCKED_CREDENTIAL_SEAM`, and 033/034 closed the pinned startup/plugin source graph while `credential_provider` remained blocked. It was removed from `main` on 2026-08-29; the nine spec packages, their modules, scripts, deployment files, and evidence are preserved in full under the git tag `archive/hermes-audit-chain`.
- `035-stock-lens-mcp-tool` is **Implemented**: append-only MCP Tool 22 `generate_stock_lens_report` exposes the existing deterministic stock lens over MCP, completing Spec 001 User Story 3 for agents. Dry-run-first side-effect; no LLM, `api_cost_ack`, network, or live market API; Tools 1-21 unchanged in name/order/signature/defaults.
