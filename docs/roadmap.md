...
> Spec 029 is **v0.19 G0 Offline Refactor** only: G1/G2/G3a are not re-approved; source and safe input/provider/overlay seams remain `BLOCKED_RUNTIME_SEAM`. A future S016 claim is only an expected high-level MCP tool-call attempt policy-blocked before dispatch, not internal Skill selection, fallback, 019 outcome, or Core execution.
...
- Latest implemented corpus package is **024-verified-report-gap-backlog**. It follows **023-historical-episode-verified-report-path**, **022-verified-research-report-coverage-index**, **021-verified-research-report-source-revalidation**, **020-verified-research-report-catalog**, **019-episode-verified-research-report-workflow**, **018-latest-episode-verified-research-report-workflow**, **017-corpus-latest-episode-deterministic-workflow**, **016-corpus-episode-completion-workflow-runner**, and **015-corpus-semantic-remediation-runner**. Packages `008` through `024` cover corpus stages through verified-report coverage, historical path, and gap backlog.
...
- `025-core-consolidation` is Implemented: a behavior-frozen internal consolidation (path-safety structure single-sourced, weak run-report writer deduplicated, `mcp_server.py` split into a facade over `mcp_runtime` + four `mcp_tools_*` groups, registry-derived docs-count checker, shared `tests/conftest.py` fixture with `PODCAST_INGEST_DATA_DIR` override). The registry had exact 21 tools with unchanged order at the time.
- `026-hermes-mcp-integration` is **Blocked**: single-instance loopback Streamable HTTP, pinned non-root sidecar deployment, recoverable OpenAB/Hermes config and four-Skill synchronization, exact-21 direct-safe validation, boolean-only endpoint-equality evidence, and migration/rollback runbook are present. C6 is PASS-current after both required reviewers and the single live v2 run. C7 remains unproved; Hermes v0.20.0 tag `v2026.8.3` hooks are only an uninstalled candidate because canonical fallback evidence and an approved safe projection are absent. Validator/inference reruns, session dumps/raw responses, and current-plan upgrade/hooks are prohibited, so status cannot become Implemented. Spec 027 contract layer is complete (offline assurance only); actual Hermes runtime routing is BLOCKED/not_evaluated and is not a runtime PASS.
- `027-hermes-skill-routing-contracts` is complete as an offline assurance package for closed Skill routing/protocol and safe evidence. It does not evaluate runtime routing, which remains BLOCKED/not_evaluated rather than runtime PASS.
- `028-hermes-runtime-skill-routing-observation` is complete as a pinned-source offline capability gate. Spec 028 capability gate is complete and correctly terminates at BLOCKED_CAPABILITY for Hermes v0.20.0 tag v2026.8.3; no upgrade, Skill sync, hooks, collector, inference, or runtime observation was performed. C6 remains PASS-current and was not rerun; actual Hermes Skill routing remains BLOCKED/not_run.
- `029-hermes-blocked-tool-attempt-runtime-smoke` is **Offline Implemented only**. It adds fail-closed source, scenario, synthetic plugin, non-forwarding interposer, read-only preflight, and rollback projections. The current terminal state is `BLOCKED_RUNTIME_SEAM`; live preflight was not run, G2-G3 are not authorized, and no Hermes/Docker/MCP runtime action occurred.
- `030-hermes-g1r-offline-remediation` is **Offline Implemented static-only**. It adds a factory-provenanced controller-plan evaluator over predecessor safe projections, exact tmpfs/no-argv/no-log/no-durable/no-TTY/no-shell/no-host-port facts, opaque credential passthrough, and rollback quarantine. Its terminal evidence remains `runtime_status=not_run`; G2/G3a/live actions are unauthorized, and no Docker/Hermes/network/MCP transport/inference/C6 action occurred.
- Spec 031 is **Offline Implemented** as immutable predecessor `031-hermes-g2-credentialless-activation-gate` with terminal `BLOCKED_CREDENTIAL_SEAM`. Its definition-only fixture was not built, run, or inspected.
- Spec 032 is **Offline Implementation Staged** as `032-hermes-g2-offline-attempt-executor`: sealed offline authority/lease, fake-driver/factory-issued in-memory single-process ledger executor, closed Docker operation/metadata contracts, and static future declaration only. Production source remains `BLOCKED_CREDENTIAL_SEAM` with `runtime_status=not_run`; fake-driver `PASS_OFFLINE_EXECUTOR_CONTRACT` is not live activation. Required reviews and the unique offline verifier remain pending; G2/H4/Docker/Hermes/C6 actions remain unauthorized. A future real Docker adapter, production ledger, and live approval are outside this closure.
- Spec 033 is **Pinned Source Audit Implemented**: it saved the exact 17-file official `NousResearch/hermes-agent` commit `b7a05b6b6f509d14f708a2fe7b7c1d3559396ef6` bundle, pins independent path-to-blob authority, atomically publishes acquisition evidence, and proves bounded plugin-loader edges by AST. It classifies `model_tools.py` as an import-time discovery caller and `hermes_cli/main.py`/`hermes_cli/oneshot.py` as call-time candidates. Its terminal is `SPEC033_PINNED_SOURCE_AUDIT_IMPLEMENTED` plus `BLOCKED_SOURCE_GRAPH`, not source readiness: directory/entry-point plugin execution and complete config/credential/provider ordering are unresolved outside the fixed allowlist. A detached review root seals reviewed artifacts before and after tests without a self-hash cycle. `runtime_status=not_run`, `live_actions_authorized=false`; no Hermes/Docker/MCP/provider/inference/config/credential action occurred and the final verifier remains for Main after reviews.
- Spec034 Task #82 current terminal is **startup/plugin closed; credential_provider BLOCKED; overall BLOCKED**: H2 remains exactly 20 official paths at frozen H1 digest `90ba45ccf11bbcbf446f7d16904964073e84837a04aaaa0c6f4887d3ea75109d`. Typed child execution records exact-match the union of manifest-reviewed mutable artifacts, detached external manifest/root authority, 20 H2 source-bundle/literal-identity records, and three predecessor-boundary-pinned Spec033 records; no self-cycle, extra, missing, link, or category mismatch is accepted. The finite reviewer-approved local purelib capability manifest is independently sealed and copied only after stable-read record validation into a capability snapshot. The sentinel child runs only `tests/test_spec_034_final_acceptance.py`, a C0–C7 pure file/AST/public-seam suite with no subprocess/socket/network call or import; runner/journal/trust meta-tests remain non-final verification. Parent post-child revalidation repeats current root, manifest, capability, H2, predecessor, and snapshot checks so permanent drift fails. Exact startup/plugin source and `manifest → loader-result → exec_module → register/context` edges are closed; credential/provider construction data flow, whole-program closure, actual activation, dynamic/external runtime paths remain blocked. `runtime_status=not_run`, `live_actions_authorized=false`; fresh re-reviews and the isolated byte-verified Main bootstrap/final invocation remain pending and unrun.
- `035-stock-lens-mcp-tool` is **Implemented**: append-only MCP Tool 22 `generate_stock_lens_report` exposes the existing deterministic stock lens over MCP, completing Spec 001 User Story 3 for agents. Dry-run-first side-effect; no LLM, `api_cost_ack`, network, or live market API; Tools 1-21 unchanged in name/order/signature/defaults. Regenerating the Spec 029 descriptor snapshot and the exact-count deny adapter moved two digest baselines that Spec 030/032 record; both are listed with current digests in `docs/install-and-porting.md` for the owning packages to re-baseline.
- `036-x-video-corpus-ingestion` is **Implemented and confirmed against a real X post**: X videos become a corpus source through an additive `PodcastProfile.source_type`, with acquisition (`yt-dlp` + `av`) as the only new capability — transcription, emission, indexing, and search are reused unchanged. Both review axes passed. Registry stayed at exactly 22 tools at the time; MCP exposure waited for Spec 040.
- `037-semantic-summary-profiles` is **Implemented, reviewed on both axes, and confirmed by one real end-to-end run**: the hardcoded finance prompt shape becomes a selectable `summary_profile` (`finance` default, plus `learning-notes`), held as pure data in `summary_profiles.py` and threaded through `create_provider` as a keyword-only argument. The rendered envelope stays frozen because `semantic_review_artifact`, `stock_lens_synthesis`, `corpus_index`, and `verified_research_lineage` read it; gooaye prompts and Markdown are pinned byte-identical. No MCP tool; the registry was exactly 22 tools at the time.
- `038-multi-document-study-guide` is **Implemented**: a new `study_guide` corpus family of four files (`00` deterministic cover plus `03`/`04`/`07` evidence-bound lecture documents) derived from an existing `learning-notes` semantic summary. Principle IV is unamended — no transcript text reaches the LLM. Not on `ARTIFACT_LADDER`; no MCP tool, and the registry was exactly 22 tools at the time.
- `039-youtube-video-corpus-ingestion` is **Implemented and live-confirmed** (2026-08-22, with one real YouTube run recorded in the package's Live Confirm Record): YouTube as a third corpus source (`source_type` / `seed_source` = `yt-video`), plus the Spec 036 seam closures — remediation no longer suggests `download_episode.py` for `x-video` / `yt-video` missing audio, local transcription uses the seed/index title, and the episode-ref alphabet gains `_`. Core plus a thin CLI; no MCP tool and no new dependency; the registry was exactly 22 tools at the time.
...
- Historical snapshot before SPEC 030: the next unused feature package number is **030**. This preserves the Spec029 offline contract wording and is not the current status above.
...
- **Spec034 Task #82 repair:** **startup/plugin closed; credential_provider BLOCKED; overall BLOCKED**. H2 stays at exactly 20 frozen paths. The typed execution snapshot binds reviewed, detached, H2, and predecessor authority without a manifest self-cycle; sealed local capability records authorize a separate purelib capability snapshot. The `-I -S` child installs verified sentinel guards before pytest import, allows only the C0–C7 final-safe acceptance target, rejects category/shadow/extra/link/missing records, and the parent revalidates current authority after child completion. All prior roots are not approval evidence. Fresh code and architecture re-reviews remain pending; the isolated documented Main-only bootstrap/final command remains unrun.
