...
| MCP tool registry / docs 對齊 | `tests/test_mcp_tool_registry_contract.py` | Current MCP registry 恰好暴露 19 個 reviewed tools；Tools 17–19 (`query_verified_research_report_catalog`, `revalidate_verified_research_report_sources`, `query_verified_research_report_coverage`) 均 append-only read-query（無 `confirm` / ack）；Tools 1–18 contracts/order 在 Tool 19 加入後保持；JSON envelope shape；README 與 `docs/mcp-usage.md` 列出全部 tools |
