...
| MCP tool registry / docs 對齊 | `tests/test_mcp_tool_registry_contract.py` | MCP 恰好暴露 16 個 reviewed tools；原有 15 個 contract/order 保持不變；side-effect tools 預設 `confirm=False`；JSON envelope shape；README 與 `docs/mcp-usage.md` 列出全部 tools；016–019 wrappers 只暴露 bounded Core inputs |
