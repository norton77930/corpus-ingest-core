...
| MCP tool registry / docs 對齊 | `tests/test_mcp_tool_registry_contract.py` | Current MCP registry 恰好暴露 17 個 reviewed tools；Tool 17 `query_verified_research_report_catalog` append-only，019 historical registry 為 16 tools，既有 Tools 1–16 contract/order 保持不變；catalog 是 read-query（非 side-effect，無 `confirm` / ack）；JSON envelope shape；README 與 `docs/mcp-usage.md` 列出全部 tools；016–020 wrappers 只暴露 bounded Core inputs |
