...
| MCP tool registry / docs 對齊 | `tests/test_mcp_tool_registry_contract.py` | MCP 恰好暴露 13 個 reviewed tools；舊 12 個 contract 保持不變；side-effect tools 預設 `confirm=False`；JSON envelope shape；README 與 `docs/mcp-usage.md` 列出全部 tools；016 completion wrapper 只暴露 bounded Core inputs |
