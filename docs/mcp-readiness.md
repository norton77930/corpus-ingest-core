...
> The Hermes sidecar audit chain (specs 026-034) terminated at BLOCKED and was removed from `main` on 2026-08-29. Its deployment files, validators, Skill-sync tooling, and evidence are preserved in full under the git tag `archive/hermes-audit-chain`.
...
Phase 5A 補齊 Codex / Claude 類 MCP client setup 文件與本機 readiness validation script。同一個 `FastMCP` instance 另有 loopback-only Streamable HTTP runner（`scripts/run_mcp_http_server.py`），stdio 路徑不受影響。
...
- Codex / Claude 類本機 client 使用 stdio；另有 loopback-only Streamable HTTP runner。沒有 legacy SSE、port publication 或 automatic port fallback。
- 尚未處理長時間任務的進度回報或取消機制。
