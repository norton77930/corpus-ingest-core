...
- corpus status and remediation: `corpus_episode_intake.py` bootstraps one RSS-resolved episode into local seed metadata; `corpus_index.py` scans local per-episode artifacts, seed metadata, and semantic review metadata, then writes deterministic JSON/Markdown under `data/corpus/{podcast_id}/`; `corpus_remediation_plan.py` refreshes that index and derives ordered remediation actions without executing them; `corpus_audio_download_runner.py` refreshes the plan, previews missing-audio ready actions by default, and only executes one explicitly confirmed audio download; `corpus_remediation_runner.py` refreshes the plan, previews deterministic-ready actions by default, and only executes filtered deterministic core actions after confirmation; `corpus_local_transcription_runner.py` refreshes the plan, previews local-audio transcript gaps by default, and only executes one explicitly confirmed local transcription; `corpus_episode_workflow_runner.py` coordinates the next safe fresh-episode stage across intake, audio download, local transcription, and deterministic remediation without adding new stage behavior.
...
16. Corpus remediation runner refreshes the remediation plan, dry-runs selected/skipped/blocked/excluded metadata without writing run reports, and writes `data/corpus/{podcast_id}/corpus-remediation-run.json` and `.md` only for confirmed, episode- or family-filtered deterministic execution.
17. Corpus local transcription runner refreshes the remediation plan, dry-runs local audio plus transcript-missing candidates without writes, and writes `data/corpus/{podcast_id}/corpus-local-transcription-run.json` and `.md` only after a confirmed single-episode transcription attempt.
18. Corpus audio download runner refreshes the remediation plan, dry-runs missing-audio ready candidates without RSS, network, downloader, audio, or report writes, and writes `data/corpus/{podcast_id}/corpus-audio-download-run.json` and `.md` only after a confirmed single-episode download attempt or explicit selection rejection.
19. Corpus fresh episode workflow runner dry-runs the next safe stage for `latest` or one explicit episode without workflow report writes, then confirmed `stage=next` execution dispatches to exactly one existing 013, 012, 011, or 010 core runner and writes `data/corpus/{podcast_id}/corpus-episode-workflow-run.json` and `.md`.
20. MCP tools expose read/search, selected side-effect tools, and consolidated research workflow; MCP completion warns about stale cache but does not rebuild automatically.
...
- Corpus remediation runner is dry-run first and filter-bounded: dry-run writes no run report artifacts, confirmed execution requires an episode or action-family filter, v1 executes only deterministic local families, and it must not execute download, transcription, semantic, LLM, RSS/network, SQLite cache rebuild, `.env`, MCP, stock-lens, or synthesis behavior.
- Corpus local transcription runner is dry-run first and single-episode bounded: dry-run writes no reports or transcript outputs, confirmed execution requires one episode, passes an explicit local audio path to transcription, and must not download, repair corrupt/partial transcripts, call LLM/MCP/network, or rebuild cache automatically.
- Corpus audio download runner is dry-run first and single-episode bounded: dry-run writes no reports or audio outputs and does not read RSS, call network, or call the downloader; confirmed execution requires one episode, calls only the existing downloader, omits source URLs/query strings/secrets/tracebacks, and must not transcribe, run downstream remediation, call LLM/MCP, or rebuild cache automatically.
- Corpus fresh episode workflow runner is dry-run first and one-stage bounded: dry-run writes no workflow report and executes no stage; confirmed execution requires `stage=next`, dispatches to exactly one existing corpus runner, writes only the latest workflow report plus selected stage artifacts, and must not execute semantic/LLM, MCP registry, cache rebuild, stock-lens, batch, or non-selected stages.
