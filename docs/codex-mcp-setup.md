...
- The local reviewed registry has exactly 20 tools. Tool 20, `suggest_historical_verified_report_next_step`, is append-only after unchanged Tools 1–19 and is a read-query next-step suggestion for one named historical episode; no confirm/ack. Tool 19, `query_verified_research_report_coverage`, remains coverage join. Tool 18 remains exact-locator offline revalidation. Tool 17 retains its offline manifest-first list/search/inspect contract and `source_currentness_status=not_evaluated` inspect result.
...
tool。目前 registry 已由後續 packages append 到 exact 20，且該 completion
