...
The local reviewed registry has exactly 18 tools. Tool 18 is append-only: Tools 1–17 keep their contracts/order. It is an offline read-only exact-locator source-revalidation query, not a side-effect: no `confirm` or acknowledgement is required. It accepts only `podcast_id`, `episode_ref`, and lowercase 64-hex `source_digest`; it has no path/output/latest/limit/query/provider/network input and performs no writes. Tool 17 retains its offline read-only manifest-first list/search/inspect contract, including `source_currentness_status=not_evaluated` for inspect.
