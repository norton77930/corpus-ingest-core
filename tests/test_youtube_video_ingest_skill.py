...
def test_youtube_video_skill_has_only_portable_required_frontmatter():
...
def test_youtube_video_skill_requires_the_ordered_human_control_protocol():
    skill = _skill_text()
    required_steps = [
        "`ingest_youtube_video` with the exact `url` the user gave and `confirm=false`",
...
def test_youtube_video_skill_never_uses_an_automatic_or_local_fallback_path():
    skill = _skill_text()
