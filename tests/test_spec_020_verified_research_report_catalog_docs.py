...
def test_spec_020_implemented_docs_and_handoff_keep_current_contracts() -> None:
    ...
    assert "共用單一 FastMCP 的 stdio 與 loopback Streamable HTTP sidecar（目前恰好 24 個 reviewed tools）" in handoff
    assert "The local stdio registry has exact 24 reviewed tools." in handoff
