...
def test_018_verified_research_report_docs_and_agent_surface_contract():
    feature = ROOT / "specs" / "018-latest-episode-verified-research-report-workflow"
    spec = _read(feature / "spec.md")
    plan = _read(feature / "plan.md")
    data_model = _read(feature / "data-model.md")
    design = _read(feature / "design.md")
    quickstart = _read(feature / "quickstart.md")
    requirements = _read(feature / "checklists" / "requirements.md")
    ...
    for text in (spec, plan, data_model, design, quickstart, requirements, safety, contract):
        assert "expected_episode_ref" in text
        assert "exact `api_cost_ack`" in text
        assert "no live market API" in text
        assert "no investment advice" in text
    assert "strict zero-write" in spec
    assert "semantic review" in spec
    assert "atomic" in design
    assert "source digest" in data_model
    ...
    assert "exact 21 tools" in architecture
    assert "018-latest-episode-verified-research-report-workflow" in handoff
    assert "21 reviewed tools" in handoff
