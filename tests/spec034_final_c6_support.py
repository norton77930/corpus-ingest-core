"""Sealed, sentinel-safe C6 synthetic verified-report public workflow."""
...
def _spec034_use_tmp_dirs(monkeypatch, tmp_path: Path) -> None:
...
def _spec034_write_json(path: Path, payload: dict) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return path

def _spec034_write_completed_artifacts(
    monkeypatch,
    tmp_path: Path,
    *,
    ...
    _spec034_use_tmp_dirs(monkeypatch, tmp_path)
    title = "EP700 Alpha"
...
def _spec034_independent_source_digest(assembly) -> str:
...
def _record_spec034_lineage(*, stock_query: str) -> None:
    from podcast_ingest_core import storage
...
def _spec034_snapshot_config_bytes() -> None:
    """Read each C6 relative config from the child snapshot's current directory."""
    approved = {
        "config/industry_chain_mappings.yaml": "55f722998321dbd3ea02e669a8fccece51199ad1364edbf8bf1c948f5f1c0d1a",
        "config/external_data_boundary.yaml": "c4adecfa835998091c3ef137a706d5d79acc5db0d2ad8b2c99757636ef6eb6b8",
        "config/gooaye_lens.yaml": "3b45f20765e2bcfa0397627526758ae2b82700dacae21ff425efc4566b4c07b6",
    }
    for relative, digest in approved.items():
        assert hashlib.sha256(Path(relative).read_bytes()).hexdigest() == digest


def run_c6_public_workflow(monkeypatch, tmp_path: Path) -> None:
    from podcast_ingest_core import storage
    ...
    _spec034_write_completed_artifacts(monkeypatch, tmp_path, stock_query="NVDA")
    ...
    _record_spec034_lineage(stock_query="NVDA")
    ...
    assert _spec034_independent_source_digest(assembly) == first.source_digest
    ...
    for index, source_artifact in enumerate(assembly.source_artifacts):
        ...
        assert _spec034_independent_source_digest(MutatedAssembly()) != first.source_digest, source_artifact.role
