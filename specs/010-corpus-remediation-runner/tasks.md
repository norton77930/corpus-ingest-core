...
- [X] T005 Add corpus remediation run filter, counts, row, result, and warning models in `src/corpus_ingest_core/models.py`
- [X] T006 Add corpus remediation runner error type in `src/corpus_ingest_core/errors.py`
- [X] T007 Add `CorpusRemediationRunAssetPaths` and `corpus_remediation_run_asset_paths(podcast_id)` in `src/corpus_ingest_core/storage.py`
- [X] T008 Create `src/corpus_ingest_core/corpus_remediation_runner.py` with public function signature, deterministic allowlist constants, excluded family constants, and no-op-safe module skeleton
- [X] T009 Export runner public function, error, and model types from `src/corpus_ingest_core/__init__.py`
...
- [X] T016 [US1] Implement refreshed remediation plan loading and empty corpus dry-run handling in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T017 [US1] Implement deterministic action selection, exclusion reasons, blocked rows, skipped rows, and dry-run counts in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T018 [US1] Implement dry-run serialization result with null report paths and no artifact writes in `src/corpus_ingest_core/corpus_remediation_runner.py`
...
- [X] T028 [US2] Implement confirmed execution filter validation and `max_actions` validation in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T029 [US2] Implement execution dispatch for `extractive_summary`, `mentions`, `episode_intelligence`, `industry_mapping`, and `external_boundary` using existing core functions in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T030 [US2] Implement output path extraction and `executed` versus `reused` outcome mapping in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T031 [US2] Implement deterministic confirmed JSON and Markdown run report writers in `src/corpus_ingest_core/corpus_remediation_runner.py`
...
- [X] T040 [US3] Implement per-action exception capture and failure row serialization without traceback leakage in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T041 [US3] Implement same-run failed-dependency skip handling in `src/corpus_ingest_core/corpus_remediation_runner.py`
- [X] T042 [US3] Harden JSON, Markdown, stdout, and stderr serialization against raw transcript, evidence, semantic body, prompt, raw LLM output, secrets, market facts, and investment advice in `src/corpus_ingest_core/corpus_remediation_runner.py` and `scripts/run_corpus_remediation.py`
