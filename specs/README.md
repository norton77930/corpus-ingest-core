...
> The Hermes sidecar audit chain (specs 026-034) terminated at BLOCKED and was removed from `main` on 2026-08-29. Its nine spec packages, the vendored pinned upstream snapshots, the deployment files, and every digest/receipt artifact are preserved in full under the git tag `archive/hermes-audit-chain`.
...
- `026`–`034` were the Hermes sidecar audit chain: loopback Streamable HTTP + non-root sidecar (026), offline Skill routing/protocol contracts (027), the pinned-source runtime capability gate (028), the blocked-tool-attempt smoke and its offline successors (029–032), and the pinned v0.19 source-graph audits (033–034). The chain **terminated at BLOCKED** — 026 never obtained C7 (safe Hermes runtime-routing evidence), 031/032 stayed at `BLOCKED_CREDENTIAL_SEAM`, and 033/034 closed the startup/plugin source graph while `credential_provider` remained blocked. It was removed from `main` on 2026-08-29 and is preserved in full under the git tag `archive/hermes-audit-chain`.
...
- `043-workflow-derivation-mcp`: **Implemented.** Appends Tool 25 `derive_workflow_bundle`, a thin wrapper over Spec 042's `run_workflow_derivation`, so the `05`/`06` derivation stops being terminal-only. Not the Spec 040/041 envelope: preview here constructs no provider, so it declares `network_read=false` where those two declare `true`. The exact `api_cost_ack` gate stays in Core and is forwarded unchanged. Provider, model, endpoint, credential, and `workflow_context` parameters are deliberately absent from the MCP surface — the last of those names a file whose contents reach an LLM prompt. Registry 24 → 25.
...
- MCP exposed: packages `003`, `005`, `016`, `017`, `018`, `019`, `020`, `021`, `022`, and `035`.
