...
- [X] T005 Add local transcription run filter, counts, row, result, and warning models in `src/corpus_ingest_core/models.py`
- [X] T006 Add local transcription runner error type in `src/corpus_ingest_core/errors.py`
- [X] T007 Add `CorpusLocalTranscriptionRunAssetPaths` and `corpus_local_transcription_run_asset_paths(podcast_id)` in `src/corpus_ingest_core/storage.py`
- [X] T008 Create `src/corpus_ingest_core/corpus_local_transcription_runner.py` with public function signature, run mode constants, transcript selection constants, and no-op-safe module skeleton
- [X] T009 Export local transcription runner public function, error, and model types from `src/corpus_ingest_core/__init__.py`
...
- [X] T017 [US1] Implement refreshed remediation plan loading and empty corpus dry-run handling in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T018 [US1] Implement local transcription selection for transcript ready actions with existing local audio path and transcript status `missing` in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T019 [US1] Implement skipped row reasons for missing audio, unsafe transcript states, non-transcript families, blocked actions, and non-ready source statuses in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T020 [US1] Implement dry-run result serialization with null report paths and no artifact writes in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
...
- [X] T030 [US2] Implement confirmed execution episode validation and pre-transcription rejection in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T031 [US2] Implement transcription dispatch using existing core function with explicit local `audio_path`, `force=False`, and runtime options in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T032 [US2] Implement transcript output path extraction and `executed` versus `reused` outcome mapping in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T033 [US2] Implement deterministic confirmed JSON and Markdown run report writers in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
...
- [X] T042 [US3] Implement per-episode transcription exception capture and failure row serialization without traceback leakage in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
- [X] T043 [US3] Harden JSON, Markdown, stdout, and stderr serialization against raw transcript, prompt, raw LLM output, secrets, traceback bodies, market facts, and investment advice in `src/corpus_ingest_core/corpus_local_transcription_runner.py` and `scripts/run_corpus_local_transcription.py`
- [X] T044 [US3] Add manual cache stale warning metadata for confirmed writes without calling `rebuild_cache` in `src/corpus_ingest_core/corpus_local_transcription_runner.py`
