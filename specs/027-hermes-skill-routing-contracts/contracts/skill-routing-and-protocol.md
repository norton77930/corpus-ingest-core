...
The four tool names are `run_corpus_episode_completion_workflow`, `run_corpus_latest_episode_deterministic_workflow`, `run_latest_episode_verified_research_report_workflow`, and `run_episode_verified_research_report_workflow`. This contract introduces no tool of its own. It was written against the exact 21-tool registry of its time; the registry later became 22 tools, and Tool 22 `generate_stock_lens_report` (spec 035) is outside this routed four-Skill set.